...
public class UserService implements UserDao {
    ...
    public User registerUser(User user) {
        ...
        String sql = "INSERT INTO Users (id,firstName,lastName,email,password,phoneNumber,city,state,country) VALUES (?,?,?,?,?,?,?,?,?)";
        ...
        int rowsAffected = jdbcTemplate.update(sql, userId, user.getFirstName(), user.getLastName(), user.getEmail(), password,
                user.getPhoneNumber(), user.getCity(), user.getState(),user.getCountry());
    ...
    public User loginUser(User tempUser) {
        ...
        public Integer updateUserWithoutImage (UUID userId, User user){
            String sql = "UPDATE Users Set email=?,bio=?,city=?,state=? WHERE id=?";
            return jdbcTemplate.update(sql, user.getEmail(), user.getBio(), user.getCity(), user.getState(), userId);
        }

        @Override
        public Integer updateUserWithImage (UUID userId, String email, String twitter, String city, String state, String
        bio, MultipartFile image, String imageType) throws IOException {
            String sql = "UPDATE Users Set email=?,bio=?,city=?,state=?,image=?,imageType=? WHERE id=?";
            return jdbcTemplate.update(sql, email, bio, city, state, image.getBytes(), imageType, userId);
        }
    ...
    @Override
    public HashMap<String, Object> grabUsers(String searchStr,String filterConditions,String numPerPage,String orderByCondition) {
        try {
            String sql;
            if(filterConditions.length() > 0 && orderByCondition.length() > 0){
                sql = "SELECT * FROM Users WHERE lastName LIKE ? " + filterConditions + " " + orderByCondition + " LIMIT " + numPerPage;
            }else if(filterConditions.length() > 0){
                sql = "SELECT * FROM Users WHERE lastName LIKE ? " + filterConditions + "ORDER BY createdAt DESC LIMIT " + numPerPage;
            }else{
                sql = "SELECT * FROM Users WHERE lastName LIKE ? ORDER BY createdAt DESC LIMIT " + numPerPage;
            }
            String countSql = "SELECT COUNT(id) AS itemCount FROM Users WHERE title LIKE ? " + (filterConditions.length() > 0 ?
            ...
            List<User> tempList = jdbcTemplate.query(sql,new Object[]{"%" + searchStr + "%"},objectMapper.mapUser());
    ...
    @Override
    public List<User> paginate(Integer currentPage, boolean earlier, boolean lastPage, Integer skipped, Integer idxBound,
                               String filterConditions,String numPerPage,String searchStr,String orderByCondition) {
        String sql;
        String lastPageStr;
        String skippedStr;
        if(filterConditions.length() > 0 && orderByCondition.length() > 0){
            sql = "SELECT * FROM Users WHERE lastName LIKE ? AND " + filterConditions;
            skippedStr = "FROM Users WHERE lastName LIKE ? AND " + filterConditions;
            lastPageStr = "SELECT * FROM Users WHERE lastName LIKE ? AND " + filterConditions + " " + (orderByCondition.contains("createdAt") ?
                    (orderByCondition.contains("ASC") ? " ORDER BY createdAt DESC " : " ORDER BY createdAt ASC ") : orderByCondition) + " LIMIT " + numPerPage;
        }else if(filterConditions.length() > 0){
            sql = "SELECT * FROM Users WHERE lastName LIKE ? AND " + filterConditions;
            skippedStr = "FROM Users WHERE lastName LIKE ? AND " + filterConditions;
            lastPageStr = "SELECT * FROM Users WHERE lastName LIKE ? AND " + filterConditions + "ORDER BY createdAt ASC LIMIT " + numPerPage;
        }else{
            sql = "SELECT * FROM Users WHERE lastName LIKE ?";
            skippedStr = "FROM Users WHERE lastName LIKE ?";
            lastPageStr = "SELECT * FROM Users WHERE lastName LIKE ? ORDER BY createdAt ASC LIMIT " + numPerPage;
        }
